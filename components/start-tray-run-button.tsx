...
export function StartTrayRunButton({ templateId, resume = false, label }: { templateId: string; resume?: boolean; label?: string }) {
  ...
  return (
    <form action={formAction}>
      ...
      <button className="btn btn-accent w-full" disabled={pending}>
        ...
        {pending ? "Opening…" : label ?? (resume ? "Resume current run" : "Start today’s run")}
      </button>
    </form>
  );
}
