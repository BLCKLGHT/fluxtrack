...
export function Brand({ href = "/", hayesIcon = false }: { href?: string; hayesIcon?: boolean }) {
  return (
    <Link href={href} className="inline-flex items-center gap-3 no-underline text-[var(--ink)]" aria-label="FluxTrack home">
      {hayesIcon ? (
        <Image src="/hayes-icon.png" alt="" width={350} height={349} className="h-11 w-11" priority />
      ) : (
        <span className="grid h-10 w-10 place-items-center rounded-[13px] bg-[var(--green)] text-sm font-black text-white shadow-sm">FX</span>
      )}
      <span>
