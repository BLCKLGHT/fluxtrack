...
export default async function WorkflowPage() {
  ...
  const [templates, profile] = await Promise.all([getTrayTemplates(demo), requireProfile(["team_viewer", "administrator"])]);
  return (
    <main id="main">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {templates.map((template) => {
          ...
          return <article className="card flex flex-col p-5" key={template.id}>
            <div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black tracking-[-.04em]">{template.tray_code}</h2><p className="muted mt-1 text-sm">{template.tray_name} · {template.source}</p>{template.is_demo && <span className="mt-2 inline-block rounded-full bg-[#fff1b8] px-2 py-1 text-[10px] font-black uppercase">Demo</span>}</div>{open ? <TrayStatusBadge status={open.status} /> : <span className="rounded-full bg-[#e7f4ed] px-3 py-1 text-xs font-extrabold text-[var(--green)]">Ready</span>}</div>
